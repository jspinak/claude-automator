 ...
 * Provides detailed logging and visual debugging for pattern matching problems.
 */
@Slf4j
@Component
public class PatternMatchingDiagnostics {
    ...
    public void diagnoseStateImage(StateImage stateImage, String context) {
        log.info("╔════════════════════════════════════════════════════════════════════════════╗");
        log.info("║           PATTERN MATCHING DIAGNOSTICS - {}                                ", context);
        log.info("╚════════════════════════════════════════════════════════════════════════════╝");
    ...
    private void logDPISettings() {
        ...
        log.info("Settings.AlwaysResize effective: {} (1.0 = no resize, < 1.0 = downscale, > 1.0 = upscale)", 
                Settings.AlwaysResize);
        
        // Get screen info
        Screen screen = new Screen();
        log.info("Screen dimensions: {}x{}", screen.w, screen.h);
        log.info("Screen DPI: {} (may be virtual/scaled)", java.awt.Toolkit.getDefaultToolkit().getScreenResolution());
        
        // Check Windows DPI
        if (System.getProperty("os.name").toLowerCase().contains("windows")) {
            try {
                // Try to get Windows scaling
                double scaleX = java.awt.GraphicsEnvironment.getLocalGraphicsEnvironment()
                    .getDefaultScreenDevice()
                    .getDefaultConfiguration()
                    .getDefaultTransform()
                    .getScaleX();
                double scaleY = java.awt.GraphicsEnvironment.getLocalGraphicsEnvironment()
                    .getDefaultScreenDevice()
                    .getDefaultConfiguration()
                    .getDefaultTransform()
                    .getScaleY();
                log.info("Windows display scaling detected: {}% x {}%", (int)(scaleX * 100), (int)(scaleY * 100));
            } catch (Exception e) {
    ...
    private void logStateImageDetails(StateImage stateImage) {
        ...
        for (int i = 0; i < stateImage.getPatterns().size(); i++) {
            ...
            if (pattern.getImage() != null && pattern.getImage().getBufferedImage() != null) {
                ...
                log.info("  - Expected scaled size at {}: {}x{} pixels", 
                    Settings.AlwaysResize,
    ...
    private PatternTestResult testPattern(Pattern pattern, int index, StateImage stateImage) {
        ...
        try {
            ...
            File screenshotFile = new File(DEBUG_DIR, String.format("screen_%s_%d.png", timestamp, index));
            ImageIO.write(screenshot, "png", screenshotFile);
            log.info("Screenshot saved: {}", screenshotFile.getAbsolutePath());
            
            // Save pattern for comparison
            File patternFile = new File(DEBUG_DIR, String.format("pattern_%s_%d.png", timestamp, index));
    ...
    private void testWithMultipleSimilarities(Pattern pattern, BufferedImage screenshot, PatternTestResult result) {
        log.info("");
        ...
        for (double threshold : thresholds) {
            try {
                ...
                org.sikuli.script.Pattern sikuliPattern = new org.sikuli.script.Pattern(pattern.getImage().getBufferedImage());
                ...
                if (finder.hasNext()) {
                    ...
                    log.info("  Threshold {}: FOUND with score {} at [{}, {}]", 
                        threshold, score, match.x, match.y);
    ...
    private void testWithBrobotAction(StateImage stateImage, int patternIndex, PatternTestResult result) {
        log.info("");
        ...
        try {
            ...
            PatternFindOptions options = new PatternFindOptions.Builder()
                .setSimilarity(Settings.MinSimilarity)
                .build();
            ...
            if (actionResult.isSuccess()) {
                ...
                actionResult.getMatchList().forEach(match -> {
                    log.info("    - Match at [{}, {}] with score {}", 
                        match.x(), match.y(), match.getScore());
                });
            } else {
    ...
    private void testDirectSikuliX(Pattern pattern, BufferedImage screenshot, PatternTestResult result) {
        log.info("");
    ...
    private void testSikuliXWithCurrentSettings(Pattern pattern, BufferedImage screenshot, String description) {
        try {
            Finder finder = new Finder(screenshot);
            org.sikuli.script.Pattern sikuliPattern = new org.sikuli.script.Pattern(pattern.getImage().getBufferedImage());
            finder.find(sikuliPattern);
            ...
            if (finder.hasNext()) {
                Match match = finder.next();
                log.info("    {} - FOUND at [{}, {}] with score {}", 
                    description, match.x, match.y, match.getScore());
            } else {
    ...
    private void generateVisualComparison(StateImage stateImage, List<PatternTestResult> results, String context) {
        try {
            String timestamp = LocalDateTime.now().format(TIMESTAMP);
            ...
            for (PatternTestResult result : results) {
                if (result.bestMatch != null) {
                    ...
                    g.drawRect(result.bestMatch.x, result.bestMatch.y, 
                        result.bestMatch.w, result.bestMatch.h);
                    
                    // Draw label
                    g.setColor(Color.YELLOW);
                    g.fillRect(result.bestMatch.x, result.bestMatch.y - 20, 150, 20);
                    g.setColor(Color.BLACK);
                    g.drawString(String.format("%s: %.2f", result.patternName, result.bestScore),
                        result.bestMatch.x + 5, result.bestMatch.y - 5);
                }
            }
    ...
    private void logSummary(List<PatternTestResult> results) {
        ...
        for (PatternTestResult result : results) {
            if (result.bestMatch != null) {
                found++;
                log.info("✓ Pattern[{}] '{}': FOUND with score {} at threshold {}", 
                    result.patternIndex, result.patternName, result.bestScore, result.bestThreshold);
            } else {
                notFound++;
                log.info("✗ Pattern[{}] '{}': NOT FOUND", 
                    result.patternIndex, result.patternName);
            }
        ...
        for (PatternTestResult result : results) {
            if (result.bestMatch != null && result.patternWidth > 0) {
                ...
                if (Math.abs(actualRatio - Settings.AlwaysResize) > 0.1) {
                    log.warn("⚠ Possible DPI scaling mismatch for pattern '{}'", result.patternName);
                    log.info("  Expected ratio: {}, Actual ratio: {}", 
                        Settings.AlwaysResize, actualRatio);
