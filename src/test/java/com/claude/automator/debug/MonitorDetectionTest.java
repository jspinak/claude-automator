 ...
 */
public class MonitorDetectionTest {
    ...
    public void detectMonitorConfiguration() {
        ...
        for (int i = 0; i < numScreens; i++) {
            ...
            System.out.printf("  Current ROI: x=%d, y=%d, w=%d, h=%d%n", 
        ...
        for (int i = 0; i < devices.length; i++) {
            ...
            System.out.printf("  Bounds: x=%d, y=%d, w=%d, h=%d%n",
                bounds.x, bounds.y, bounds.width, bounds.height);
            
            // Check for virtual screen offset
            if (bounds.x != 0 || bounds.y != 0) {
                System.out.printf("  WARNING: Screen has offset! x=%d, y=%d%n", 
                    bounds.x, bounds.y);
            ...
            System.out.printf("  Display Mode: %dx%d, %d-bit, %dHz%n",
        ...
        System.out.printf("Virtual desktop bounds: x=%d, y=%d, w=%d, h=%d%n",
        ...
        if (!allEqual) {
            System.out.println("WARNING: Different construction methods produce different regions!");
