 ...
 */
public class VisualizeMatchesTest {
                ...
                for (double threshold : thresholds) {
                    ...
                    while (finder.hasNext()) {
                        ...
                        if (matchCount <= 10) {
                            ...
                            g.setColor(color);
                            g.drawString(String.format("%.2f", match.getScore()), match.x + 2, match.y - 5);
                        }
                        ...
                        if (matchCount <= 5) {
                            ...
                            if (match.x >= 0 && match.y >= 0 && 
                                ...
                                BufferedImage matchedRegion = screenImg.getSubimage(
                                    match.x, match.y, match.w, match.h);
                                
                                String matchFilename = String.format("matched_%s_t%.2f_m%d_s%.3f.png",
                                    patternName, threshold, matchCount, match.getScore());
                                File matchFile = new File(outputDir, matchFilename);
                                ImageIO.write(matchedRegion, "png", matchFile);
                                System.out.println("    Saved matched region to: " + matchFile.getName());
                            }
                        }
                    }
                    ...
                    String filename = String.format("screen_%s_t%.2f_%s.png", 
                        patternName, threshold, timestamp);
                    ...
                    if (threshold >= 0.90 && bestMatch != null && bestScore >= 0.90) {
                        ...
                        compareImages(patternImg, screenImg.getSubimage(
                            bestMatch.x, bestMatch.y, bestMatch.w, bestMatch.h), outputDir, patternName);
                    }
                }
            }
    ...
    private void compareImages(BufferedImage pattern, BufferedImage matched, File outputDir, String name) {
        try {
            System.out.println("Pattern: " + pattern.getWidth() + "x" + pattern.getHeight() + 
                " type=" + pattern.getType());
            System.out.println("Matched: " + matched.getWidth() + "x" + matched.getHeight() + 
            ...
            int[] sampleX = {0, pattern.getWidth()/2, pattern.getWidth()-1, pattern.getWidth()/4, 3*pattern.getWidth()/4};
            int[] sampleY = {0, pattern.getHeight()/2, pattern.getHeight()-1, pattern.getHeight()/4, 3*pattern.getHeight()/4};
            ...
            for (int i = 0; i < 5; i++) {
                ...
                System.out.printf("  (%d,%d): Pattern=%08X, Matched=%08X, %s\n",
                    x, y, patternPixel, matchedPixel,
