 ...
 */
public class ImageComparisonDebugTest {
    ...
    public void debugImageComparison() {
        ...
        try {
            ...
            System.out.println("   Num Components: " + patternImg.getColorModel().getNumComponents());
            System.out.println("   Num Color Components: " + patternImg.getColorModel().getNumColorComponents());
            ...
            System.out.println("   Region: (" + searchX + ", " + searchY + ") " + searchW + "x" + searchH);
            ...
            BufferedImage patternRGB = new BufferedImage(
                patternImg.getWidth(), 
            ...
            BufferedImage patternBGR = new BufferedImage(
                patternImg.getWidth(), 
                patternImg.getHeight(), 
            ...
            if (sikuliLoadedImg != null) {
                ...
                System.out.println("     Size: " + sikuliLoadedImg.getWidth() + "x" + sikuliLoadedImg.getHeight());
    ...
    private void analyzeImagePixels(BufferedImage img, String label) {
        ...
        for (int i = 0; i < samples.length; i++) {
            ...
            System.out.printf("     %s (%d,%d): R=%d G=%d B=%d A=%d (0x%08X)\n",
        ...
        System.out.printf("     Average color (sampled): R=%d G=%d B=%d A=%d\n",
            totalR/samplesCounted, totalG/samplesCounted, totalB/samplesCounted, totalA/samplesCounted);
    }
    ...
    private void searchForSimilarRegion(BufferedImage pattern, BufferedImage search) {
        ...
        System.out.println("   Pattern average color: RGB(" + avgPR + "," + avgPG + "," + avgPB + ")");
        ...
        for (int y = 0; y <= sh - ph; y += step) {
            for (int x = 0; x <= sw - pw; x += step) {
                ...
                int diff = Math.abs(avgRR - avgPR) + Math.abs(avgRG - avgPG) + Math.abs(avgRB - avgPB);
        ...
        if (bestX >= 0) {
            System.out.println("   Best match at (" + bestX + "," + bestY + ") with color diff: " + minDiff);
            ...
            System.out.println("   Region color: RGB(" + (rR/samples) + "," + (rG/samples) + "," + (rB/samples) + ")");
        }
    }
