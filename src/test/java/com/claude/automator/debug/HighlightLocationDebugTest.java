 ...
 */
public class HighlightLocationDebugTest {
    ...
    public void debugHighlightLocation() {
        ...
        try {
            ...
            System.out.printf("SikuliX Screen dimensions: %dx%d%n", 
                    screen.w, screen.h);
            ...
            Region brobotRegion = Region.builder()
            ...
            System.out.printf("X coordinates match: %s (Brobot=%d, Sikuli=%d)%n", 
                    xMatch, brobotRegion.getX(), sikuliRegion.x);
            System.out.printf("Y coordinates match: %s (Brobot=%d, Sikuli=%d)%n", 
                    yMatch, brobotRegion.getY(), sikuliRegion.y);
            System.out.printf("Width matches: %s (Brobot=%d, Sikuli=%d)%n", 
                    wMatch, brobotRegion.getW(), sikuliRegion.w);
            System.out.printf("Height matches: %s (Brobot=%d, Sikuli=%d)%n", 
            ...
            System.out.printf("Java AWT headless mode: %s%n", 
                    java.awt.GraphicsEnvironment.isHeadless());
            System.out.printf("Operating System: %s%n", System.getProperty("os.name"));
            System.out.printf("Display environment variable: %s%n", 
            ...
            for (int i = 0; i < numScreens; i++) {
                ...
                System.out.printf("Screen %d: %dx%d at (%d,%d)%n", 
                        i, s.w, s.h, s.x, s.y);
            }
    ...
    private void testSpecificRegion(String label, int x, int y, int w, int h) {
        ...
        // Check if they match
        boolean match = (sikuliRegion.x == x && sikuliRegion.y == y && 
                        sikuliRegion.w == w && sikuliRegion.h == h);
        System.out.printf("  Coordinates preserved correctly: %s%n", match);
        
        if (!match) {
            System.out.printf("  ERROR: Expected (%d,%d,%d,%d) but got (%d,%d,%d,%d)%n",
